...
public class JointNetwork: Module {
    ...
    public func callAsFunction(
        ...
        let encMax = encoderOutput.max().item(Float.self)
        let predMax = predictionOutput.max().item(Float.self)
        ...
        // Check projections for NaN
        let encProjMax = encProj.max().item(Float.self)
        let predProjMax = predProj.max().item(Float.self)
        ...
        let combinedMax = x.max().item(Float.self)
        ...
        let afterActivationMax = x.max().item(Float.self)
        ...
        let finalMax = x.max().item(Float.self)
...
public class CustomLSTM: Module {
    let inputSize: Int
    let hiddenSize: Int
    let numLayers: Int
    let batchFirst: Bool
